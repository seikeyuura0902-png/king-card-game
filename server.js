...
function resolveBattle(c1_orig, c2_orig) {
  const c1 = String(c1_orig).toLowerCase();
  const c2 = String(c2_orig).toLowerCase();
  if (c1 === c2) return 'draw';
  const isSpec = (c) => [KEYS.EMP, KEYS.FIRST, KEYS.SNIP, KEYS.REVO].some(k => c.includes(k));

  if (c1.includes(KEYS.SLAVE) && isSpec(c2)) return 'p1';
  if (c2.includes(KEYS.SLAVE) && isSpec(c1)) return 'p2';
  if (isSpec(c1) && isSpec(c2)) return 'draw';
  if (isSpec(c1)) return 'p1';
  if (isSpec(c2)) return 'p2';

  const getS = (c) => {
    if (c.includes('noble')) return 4;
    if (c.includes('general')) return 3;
    if (c.includes('soldier')) return 2;
    if (c.includes('citizen')) return 1;
    return 0;
  };
  const s1 = getS(c1); const s2 = getS(c2);
  if (s1 > s2) return 'p1';
  if (s2 > s1) return 'p2';
  return 'draw';
}
...
io.on('connection', (socket) => {
  socket.on('findMatch', (data) => {
    ...
    if (waitingPlayers.length > 0) {
      ...
      const createP = (n) => ({
        name: n, hand: ['noble', 'general', 'soldier', 'citizen', 'slave', 'emperor', 'first_emperor', 'sniper', 'revolutionary'],
        dead: [], assassinated: [], usedSpecial: [], killCount: 0, ready: false, selectedCard: null, greatWallActive: false
      });
      rooms[roomId] = { id: roomId, sockets: { p1: opp.socketId, p2: socket.id }, 
  ...
  socket.on('selectCard', (data) => {
    ...
    if (p1.ready && p2.ready) {
      ...
      if (res === 'p1') { 
        ...
        p1.killCount++; winId = 'p1'; 
      }
      else if (res === 'p2') { 
        p1.hand = p1.hand.filter(c => c !== p1.selectedCard); p1.dead.push(p1.selectedCard); 
        p2.killCount++; winId = 'p2'; 
      }

      // --- 能力発動条件の判定 ---
      if (winId) {
        const winner = gs.players[winId];
        const cardName = String(winner.selectedCard).toLowerCase();
        const key = [KEYS.EMP, KEYS.FIRST, KEYS.SNIP, KEYS.REVO].find(k => cardName.includes(k));
        ...
        if (key && !winner.usedSpecial.includes(key)) {
          ...
          if (key === KEYS.REVO) {
            // 革命家：3枚死んでいたら発動
            if (totalDead >= 3) trigger = true;
          } else {
            // 他の特殊カード：2枚死んでいたら発動
            if (totalDead >= 2) trigger = true;
          }
...
server.listen(PORT, '0.0.0.0', () => console.log(`Game Server Running`));
